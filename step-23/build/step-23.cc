...
{
    // まず円盤のメッシュを作る（中心 (0,0) 半径 1.0）
    GridGenerator::hyper_ball(triangulation, Point<dim>(0, 0), 1.0);

    // 楕円マニフォールドを適用
    EllipticalManifold<dim> elliptical_manifold(Point<dim>(0, 0), Tensor<1, dim>({4.0, 0.0}), 0.9);
