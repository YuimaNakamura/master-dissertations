 ...
 */
 ...
 #include <fstream>                           // ファイル読み込み用（std::ifstream）
 ...
 #include <deal.II/grid/grid_in.h>            // GridIn クラス（メッシュ読み込み用）
 ...
 namespace merge
 {
   ...
   public:
                          ...
                          const unsigned int component = 0) const override
     {
       ...
       if ((this->get_time() <= 1.0) && (p[0] < 0) && (p[1] < 40) &&
           (p[1] > -40))
         return 0.5 * std::sin(this->get_time() * 4 * numbers::PI);
       else
         return 0;
     }
   };
   ...
   public:
                          ...
                          const unsigned int component = 0) const override
     {
       ...
       if ((this->get_time() <= 0.5) && (p[0] < 0) && (p[1] < 40) &&
           (p[1] > -40))
         return (0.5 * std::cos(this->get_time() * 4 * numbers::PI) * 4 * numbers::PI);
       else
         return 0;
     }
   };
   ...
   {
       // GridIn を用いてメッシュを読み込む
       GridIn<dim> grid_in;
       grid_in.attach_triangulation(triangulation); // triangulation はメンバ変数
       std::ifstream input_file("/mnt/c/Users/yuiat/OneDrive - University of Sussex/dealii/step-23/merge/curve2.msh");
   
       Assert(dim == 2, ExcNotImplemented());  // 2次元であることを確認
       grid_in.read_msh(input_file); // MSHファイルを読み込む
   
       // メッシュ情報を出力
       std::cout << "Number of cells: " << triangulation.n_cells() << std::endl;
       std::cout << "Number of lines: " << triangulation.n_lines() << std::endl;
       std::cout << "Number of vertices: " << triangulation.n_vertices() << std::endl;
   
       // メッシュをリファイン
       triangulation.refine_global(3);
   
       std::cout << "Grid creatin and refinement completed." << std::endl;
   }
