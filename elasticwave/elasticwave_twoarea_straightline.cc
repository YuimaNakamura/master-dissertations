...
namespace ElasticWave2D
{
  ...
  {
    static double rho(const Point<dim> &p)
    {
      return (p[0] < 0) ? 1.0 : 1000.0;
    }

    static double lambda(const Point<dim> &p)
    {
      return (p[0] < 0) ? 1.0 : 10.0;
    }

    static double mu(const Point<dim> &p)
    {
      return (p[0] < 0) ? 1.0 : 10.0;
    }
  };
  ...
  void ElasticWave<dim>::assemble_system()
  {
    ...
    FEValues<dim> fe_values(fe, quadrature_formula, update_gradients | update_JxW_values | update_quadrature_points);
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        const double lambda = MaterialProperty<dim>::lambda(p);
        const double mu = MaterialProperty<dim>::mu(p);
  ...
  void ElasticWave<dim>::assemble_mass_matrix()
  {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        const double rho = MaterialProperty<dim>::rho(fe_values.quadrature_point(q));
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          for (unsigned int j = 0; j < dofs_per_cell; ++j)
          {
            cell_mass(i, j) += rho * fe_values.shape_value(i, q) * fe_values.shape_value(j, q) * fe_values.JxW(q);
          }
        }
      }
  ...
  void ElasticWave<dim>::initialize_solution()
  {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int i = 0; i < fe.n_dofs_per_cell(); ++i)
      {
        ...
        const Point<dim> p = cell->vertex(i / dim); // 簡易取得
  ...
  void ElasticWave<dim>::output_results(const unsigned int timestep) const
  {
    ...
    std::ofstream output("solution-" + std::to_string(timestep) + ".vtu");
