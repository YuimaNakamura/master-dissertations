...
void simulation_rootfft_graph() {
  ...
  for (Long64_t i = 0; i < n; ++i) {
    tree->GetEntry(i);
    if (dof_index == 1400) {
      signal.push_back(disp_x); // displacement_x に変えたい場合はこちら
    }
  }
  ...
  // --- 結果取得
  TH1D *hfft = new TH1D("hfft", "FFT of displacement_x (dof_index=1400);Frequency bin;Amplitude", N/2, 0, N/2);
