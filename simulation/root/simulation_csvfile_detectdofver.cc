...
// --- 観測点用: 与えられた座標に最も近いDoFを探して保存（変位は与えない） ---
      const Point<dim> observation_point(220000.0, 120000.0); // 例：追跡したい点
    ...
    void ElasticWave<dim>::output_results(const unsigned int timestep) const
    {
        ...
        // ヘッダ行 (自由度インデックス, x_displacement, y_displacement)
        output << "dof_index";
        for (unsigned int c = 0; c < dim; ++c)
            output << ", displacement_" << (c == 0 ? "x" : "y");
        output << "\n";

        // FESystemでdim成分連結のため、自由度インデックスをcomp毎に分割して書く
        // ここではdim=2を想定
...
///////////////////////////////////////////////
    ...
    {
        time_step();
        if (step % 1 == 0)//ここを10 == 0にしたら10ごとのファイル出力になる
        output_results(timestep_number);
    }
    }
